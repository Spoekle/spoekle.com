...
router.get('/auth', (req, res) => {
  ...
  const discordAuthUrl = `https://discord.com/oauth2/authorize?client_id=${CLIENT_ID}&response_type=code&redirect_uri=${encodeURIComponent('https://spoekle.com/api/discord/callback')}&scope=identify+guilds.members.read+email&state=${state}`;
  res.redirect(discordAuthUrl);
});

// New route for bot login
router.get('/bot-auth', (req, res) => {
  const discordBotAuthUrl = `https://discord.com/oauth2/authorize?client_id=${CLIENT_ID}&response_type=code&redirect_uri=${encodeURIComponent('https://spoekle.com/api/discord/bot-callback')}&scope=identify+guilds.members.read+email&state=bot`;
...
router.get('/callback', async (req, res) => {
  ...
  try {
    const tokenResponse = await axios.post('https://discord.com/api/oauth2/token', new URLSearchParams({
      ...
      code: code,
      redirect_uri: 'https://spoekle.com/api/discord/callback'
    }), {
    ...
    if (existingUser) {
      ...
      if (existingUser.status !== 'active') res.redirect(403, `https://spoekle.com`);
      ...
      return res.redirect(`https://spoekle.com?token=${encodeURIComponent(existingToken)}`);
    }
    ...
    if (state && state !== 'undefined') {
      ...
      if (user) {
        ...
        user.roles = ['user'];
        await user.save();
        if (user.status !== 'active') return res.redirect(403, `https://spoekle.com`);
        const userToken = jwt.sign({ id: user._id, username: user.username, roles: user.roles }, secretKey, { expiresIn: '1 day' });
        return res.redirect(`https://spoekle.com?token=${encodeURIComponent(userToken)}`);
      }
    }
    ...
    await newUser.save();
    if (newUser.status !== 'active') return res.redirect(403, `https://spoekle.com`);
